...
| Asset | Requirement |
|---|---|
| App icon | 512 × 512 PNG. A vector adaptive icon ships in the APK; the store PNG is separate. |
| Feature graphic | 1024 × 500 PNG |
| Phone screenshots | 2–8, minimum 320 px on the short side. **Requires a device or emulator, which this environment does not have.** |
| Privacy policy URL | Play requires a hosted URL. `docs/DATA_SAFETY.md` has the content; hosting it is the operator's. |

## A note on the screenshots

They cannot be faked from mockups here, and should not be: Play screenshots must show the app
as it actually is, and nothing in this repository has ever been rendered on a screen. They need
a device.
