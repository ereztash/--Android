...
| WAIVER-3 | P7 | 2026-08-25 | operator | **`ARM-EDGE` ships below its own pre-registered stopping rule.** `B2` required a candidate to remove **90%** of bidi divergence; `ARM-EDGE` removes **67%**. The arm that *did* clear the rule, `ARM-FSI-EDGE`, reaches 100% and **changes what 339 of 3,257 lines look like for users who had no problem** — a cost the rule did not price, found by a post-hoc check and recorded rather than used to renegotiate the rule. `ARM-EDGE` changes that for **zero** lines and removes **100%** of the bracket divergence, which is the complaint verified against both Gboard and SwiftKey. The operator chose the smaller fix with no collateral over the larger fix with some. Compensating controls: `BidiPinTest` protects the behaviour, the marks are `Cf` so stripping format recovers the user's text exactly, and the pair is never placed into a field whose contents the IME cannot see. The waiver ends when `M7-LAT` and `M13-BIDI-RENDER` have run on a device — **both are NOT MEASURED and neither is small.** |
