...
| GATE-API-1 | No IME API that compiles cleanly and fails at runtime (§1.1/1.3/1.4/1.6) | 120 files, 6 rules | Planted session-override, return-value branch, hardcoded backspace, blocking fetch |
| GATE-API-1 | `getInitial*` accessors only inside the privacy boundary (§1.2) | 120 files | Planted read outside the boundary |
| GATE-API-1 | Nothing typed reaches logcat or stdout | 120 files, production sources | Planted `Log.d` and `println` |
| GATE-CRYPTO-1 | No ECB, hardcoded IV/key, seeded `SecureRandom`, broken primitive | 120 files, 4 rules | Planted `AES/ECB`, fixed IV, hardcoded key, MD5, seeded random |
...
| GATE-LEARN-2 | Learning happens in exactly one place, guarded by `session.mayLearn` | 124 Kotlin source files | A second, unguarded call site |
...
| E4-APPTESTS | Automated coverage of the `:app` module | **Was zero; now 3.** `app/src/test` did not exist and Robolectric was not a dependency, while `:core` carried 303 tests and `:app` is where everything is wired. Robolectric added at **test scope only** — it cannot reach the shipped artifact | Covers service construction, and that a password field is counted restricted with `initialTextTaken` **unchanged**, plus a control that a plain field is not. Asserted on the recorded BOOLEAN, never on the text: naming `getInitialTextBeforeCursor` trips `priv.initial_text`, whose allow-list says *widening this is a privacy decision, not a refactor* — so the test was rewritten, not the gate. **Closes no device-blocked row**: shadows are a reimplementation, and `M4-DEVICE` asks what the REAL framework hands over |
