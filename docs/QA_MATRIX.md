...
| G1-SHARING | Whether the forms of one word share units | ktiv pairs **0.3186** against a random control of **0.0093** — **34.20×**, the pre-registered bar being 3× | n=5,000 per family. **Absolute Jaccard has a structural ceiling** — 0.3333 for a two-unit verb pair, 0.5000 for a one-unit-stem prefix pair — which the harness does not report. See `docs/SEGMENTATION.md` |
