...
| GATE-API-1 | No IME API that compiles cleanly and fails at runtime (§1.1/1.3/1.4/1.6) | 119 files, 6 rules | Planted session-override, return-value branch, hardcoded backspace, blocking fetch |
| GATE-API-1 | `getInitial*` accessors only inside the privacy boundary (§1.2) | 119 files | Planted read outside the boundary |
| GATE-API-1 | Nothing typed reaches logcat or stdout | 119 files, production sources | Planted `Log.d` and `println` |
| GATE-CRYPTO-1 | No ECB, hardcoded IV/key, seeded `SecureRandom`, broken primitive | 119 files, 4 rules | Planted `AES/ECB`, fixed IV, hardcoded key, MD5, seeded random |
...
| GATE-LEARN-2 | Learning happens in exactly one place, guarded by `session.mayLearn` | 123 Kotlin source files | A second, unguarded call site |
...
| E4-APPTESTS | Automated coverage of the `:app` module | **There is none.** `app/src/test` does not exist and Robolectric is not a dependency. `:core` carries 303 tests; the module that wires the IME together — `HebrewImeService.execute`, the commit path, `CorrectionController`, `DeviceEvidence`, the sensitive-field session handling — has **zero**. All 16 of its Kotlin files import `android.*`, so none is reachable by plain JUnit | Closing this needs Robolectric (a build change) or instrumented tests (device, blocked). **Not done unilaterally**: Robolectric's shadows cannot answer the framework-behaviour questions that dominate the blocked list — `M4-DEVICE` asks what the REAL framework hands over — so it would add coverage without closing a single row. That trade is the operator's to make, not mine |
