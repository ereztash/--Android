...
| GATE-API-1 | No IME API that compiles cleanly and fails at runtime (§1.1/1.3/1.4/1.6) | 121 files, 6 rules | Planted session-override, return-value branch, hardcoded backspace, blocking fetch |
| GATE-API-1 | `getInitial*` accessors only inside the privacy boundary (§1.2) | 121 files | Planted read outside the boundary |
| GATE-API-1 | Nothing typed reaches logcat or stdout | 121 files, production sources | Planted `Log.d` and `println` |
| GATE-CRYPTO-1 | No ECB, hardcoded IV/key, seeded `SecureRandom`, broken primitive | 121 files, 4 rules | Planted `AES/ECB`, fixed IV, hardcoded key, MD5, seeded random |
...
| GATE-LEARN-2 | Learning happens in exactly one place, guarded by `session.mayLearn` | 125 Kotlin source files | A second, unguarded call site |
...
| G1-SEG | A four-layer segmentation at 16,384 units: 79 closed prefixes, 3,516 lemmas + 57 feature bundles from source A's discarded columns, 12,705 BPE merges, 27 character fallbacks | **100% coverage** over 481,861 words and tokens · **1.809 units per token** on held-out conversational text | Nothing shipped reads it. Layers 1–2 are morphology; the BPE layer is not, and 71.25% of the lexicon receives subwords rather than morphology |
| G1-SHARING | Whether the forms of one word share units | ktiv **0.3186** against a control of **0.0093** — **34.20×**, the pre-registered bar being 3×. Against the two segmentations' sizes, ktiv reaches **0.36** of what they allow — the weakest of the three families | n=5,000 per family. The harness now reports a size ceiling and a design ideal computed from the licensed table: verbs reach **0.88** of ideal, prefixes **0.84**. The ceilings first derived by hand were wrong in both directions. See `docs/SEGMENTATION.md` |
| G1-AMBIGUITY | Hebrew verb forms that carry more than one analysis | Source A holds **86** feature bundles; the segmenter reaches **57**, because the surface→analysis map is many-to-one and `load_verbs()` keeps the first | **NOT a decision anyone made.** The same shape as `build_lexicon.py` reading one column of five, one level down |
| G3-BUDGET | Whether a larger budget cures `G2`'s compression penalty | **NOT ADOPTED.** Best cell is 32,768 units `shortest` at **1.944** units/token against G1's **1.809** — still worse, and 1,048,576 bytes is affordable. No cell meets every clause | 3 budgets × 2 modes, 5,000 pairs each, every bar against G1 re-measured in the same run |
| G3-COMPETITION | Why the analysis layer does not pay for itself | **The layers are not independent.** Admitting the table removes 214,648 noun/adjective forms from BPE's training residual, so BPE learns worse merges for everything that remains. Not scarcity, not ordering — competition for training data | Same run. Falsified the `shortest`-cannot-lose-by-construction claim: the guarantee held per-stem against a BPE that no longer exists in that configuration |
| G3-HARNESS | Two defects found by reading the output rather than by the harness | The row labelled *budget 16,384* has vocabulary **20,554** — `G2`'s `vocab ≤ budget` assertion was replaced by the affordability clause and lost; and `noun J/ideal` reads **1.01** because numerator and denominator average over different pair subsets | Both recorded rather than tidied. Neither changes the verdict: compression fails by 0.135 at the best cell |
| G2-NOUNADJ | Whether a CC0 noun/adjective paradigm layer earns its slots inside the 16,384-unit budget | **NOT ADOPTED.** The layer reaches **0.99–1.00 of its own design ideal** — it works — but compression rises from **1.809 to 2.060–2.168** units/token at every slot count, failing the pre-registered bar | 5 slot counts, 5,000 pairs each, every bar against N=0 re-measured in the same run. Wikidata Lexemes, CC0, dump sha256 `7f3ef4fd…83df1c` |
| G2-MEMORISATION | Why the morphological layer loses on compression | A noun form becomes `[lemma, features]` — **two units always** — while BPE at 12,705 merges had memorised the frequent ones as **one**. Given morphology is not automatically better than memorised subwords at this budget | Same run. The same shape as `D1`'s *a table covers the head*, from the opposite direction |
...
| W5-ARTIFACT | The public artifact | **`docs/FINDINGS.md` + `scripts/reproduce.sh`** — one entry point regenerating every headline number from committed inputs, each block printing its own planted-defect control first | Ordered by strength of evidence, which is **not** `PROGRAM`'s original order: `W6` and `W1` changed which claim is strongest |
| W5-ORDER | What the strongest claim turned out to be | **Not the alphabet finding `PROGRAM` nominated.** It is `W6` — AOSP's shipped Hebrew bracket fix makes every measured axis worse — because it is verifiable from public source by anyone, and measured on 3,257 real typed lines | The register finding is third, and carries its own correction on its face |
| W8-CORRECTION | Spelling correction accuracy on words a person typed | **top-1 52.60% → 51.15%, top-3 66.23% → 64.48%.** False auto-replace **0.68% → 0.60%** — *lower* | Only the source words differ; the corruption generator, discard rule, seed stream and engine are the same code. **All three pre-registered predictions falsified** |
| W8-ASYMMETRY | Why this matters | **Correction is register-robust; completion is not.** Completion moved by a factor of **2.3** between registers; correction moves **1.45 points** | Correction is edit-distance over the lexicon, which `H1` measured as covering both registers (99.16% / 95.01%). Completion depends on the bigram model, which carries its corpus's register |
| W8-HEADLINE | What happens to the published 52.60% | **It survives.** It needs a qualifier — *measured on encyclopedic source words* — not a correction | The opposite of what happened to the completion headline. **This number was not overstated** |
| W8-SELFCORRECTION | A defect inside the run | The harness first measured *offers a suggestion* and called it *false auto-replace* — **4.63%**, which against a published 0.68% reads as a 7× regression in a different quantity. Fixed to call `shouldAutoReplace`, it reproduces **0.68% exactly** | The mistake became a passing control. The weaker event is now reported beside it, not instead of it |
| W6-FUTO | Whether *offline and actually good at Hebrew* is an occupied position | **Open.** FUTO Keyboard — $11.99, offline, 100K+ installs, the only paid offline keyboard found — **has no Hebrew dictionary**, so no Hebrew correction and no Hebrew prediction | Its release notes mention Hebrew once, v0.1.27 (2024-12-25): *"Fixed some behavior when it comes to typing parentheses in Hebrew"* |
| W6-AOSP | What AOSP LatinIME actually ships for Hebrew brackets | **The key labelled `(` commits U+0029.** `values-iw/donottranslate-more-keys.xml` defines `keyspec_left_parenthesis = "(\|)"`, and `KeySpecParser`'s javadoc documents the format as `keyLabel\|keyOutputText` | Both verified from source, not inferred. FUTO's fork carries the same values. **It changes the character in the user's text**, not the rendering |
| W6-SWAPCOST | That behaviour measured against 3,257 real typed lines | **Every axis worse.** Bracket divergence **77% → 86%**, overall 29% → 31%, **fixes 0 of 952, breaks 60**, the only arm that **CORRUPTS** round-trip, and it disturbs the Hebrew-locale rendering of **18.4%** of lines against 0% for `ARM-EDGE` | Supports the hypothesis that the verified complaint — *"הסוגריים תמיד יוצאים הפוכים"* — is **caused by the fix, not its absence**. A hypothesis, not a measurement of anyone's product |
| W6-LIMITS | What W6 does not establish | **Gboard is not measured** — closed source, descends from LatinIME, may differ. `M12-GBOARD-CODEPOINT` stays NOT MEASURED and needs a device | The corpus was typed on unknown keyboards, possibly swapping ones, which makes the *changes-the-rendering* column ambiguous in a way the divergence column is not |
| W4-SCOPE | How many shipped keys have a label that is not the glyph the user sees | **Exactly two: `(` and `)`.** Every other character the three layouts emit — `- / : ; ₪ & @ " . , ? ! '` and all ten digits — renders as its own label | Measured before deciding anything. The problem is two keys wide |
| W4-RULE | The invariant now enforced | **Either the label is the glyph, or the key carries a written reason.** `Key.labelDiffersBecause`, null on every key but two; the mismatch count is pinned at 2 so a new mirrored character fails rather than being absorbed | The `GATE-CORPUS-2` bargain: a machine finds the mismatch, only a person may let it stand |
| W4-NOTRELABEL | Whether to relabel `(` to `)` | **Rejected.** `KeyboardModel.kt` records this project shipping exactly that class of mistake — mirroring the layout because the script is RTL, which *"shipped, and a user opened the keyboard and said it looked like a mirror"* | SI-1452 muscle memory against a benefit nobody has measured. **`L2-LABEL` NOT MEASURED**, needs a user |
| W4-CONTROLS | Both directions of the check demonstrated red | Reason **removed** from a mismatching key → FAILED. Reason **added** to a key whose label and glyph agree → FAILED | The second matters as much: a reason for a mismatch that does not exist is a false statement about what a key does |
| B2-SHIPPED | `ARM-EDGE` in the shipped input path | **SHIPPED 2026-08-25 on the operator's decision, against the rule's verdict.** The rule adopted `ARM-FSI-EDGE` at 100%; the operator chose `ARM-EDGE` at 67% because it changes the rendering for **zero** users who had no problem, against 339 lines for the arm that cleared the bar | Recorded as **`WAIVER-3`** with date, owner, reason and compensating controls. `P7` stays MET because the override is written down |
| B2-SENSITIVE | Whether `ARM-EDGE`'s direction marks can reach a password field | **They cannot — but by accident, and nothing asserted it until now.** `HebrewImeService.execute` calls `BidiPin.shouldPin` on every commit with no reference to the session's restriction. What saves it is a three-link chain in which no link mentions the other two: a restricted session exposes `initialTextBeforeCursor = null`, `InputContextBuffer.reset(null, _)` leaves `precedingContextKnown = false`, and `shouldPin` returns false on a null `textBefore` | `BidiPinSensitiveFieldTest` now asserts it across 5 restricted field types, with a control proving an ordinary empty field DOES pin so the assertion is not vacuous. **`docs/PRIVACY_POLICY.md` promises these fields get "the characters and nothing else", and two U+200F marks are not nothing** — changing that `null` to `""` would falsify the policy and corrupt passwords silently |
| B2-HALVES | Whether one mark would do, measured before implementing | **No. Each half alone breaks 115 previously-convergent lines.** Leading only fixes 3/952, trailing only 275/952, **both 641/952 and break 0** | The cheap implementation — one mark, committed once — was measured and is **not available** |
| B2-COST | Steady-state cost of the pair | **Zero.** Both marks are placed once, on the first Hebrew character into a known-empty field, with the cursor left between them | The obvious design is a treadmill: delete and re-commit the trailing mark every keypress. `M7-LAT` has still never run and that is not small |
| B2-BRACKETS | Whether the bracket complaint reproduces on lines a person typed | **It does — 77% of 463 real bracket-bearing lines.** `B1` reported **0 of 8** on strings I hand-built, all balanced and fully enclosing | **`B1` committed the defect it had diagnosed in the eval corpora one file earlier**: it measured on material that could not exhibit the thing it looked for. The verified market complaint is real, and `ARM-EDGE` removes 100% of it |
| B2-ARMS | Seven arms against the four-clause rule, on 3,257 typed lines | **`ARM-FSI-EDGE` fixes 952/952 (100%), breaks 0, round-trips clean — ADOPTED by the rule.** `ARM-EDGE` fixes 641/952 (67%), breaks 0. `ARM-FSI`/`ARM-LRI` fix 2% and **break 35** | First arm in this repository to clear a pre-registered bar. **Adopted means cleared its bar in the harness** — nothing ships on it |
| B2-CORRECTNESS | *Post hoc.* Divergence measures consistency, not correctness | **`ARM-FSI-EDGE` changes what 339 of 3,257 lines look like for users who had no problem** (89.6% preserved). **`ARM-EDGE` preserves 100% and changes 0** | The arm that clears the bar has a cost the rule did not price; the arm that costs nobody misses the bar. **This does not un-adopt anything** — a fifth clause is registered for the next run, not applied backwards |
| B2-PC3 | The inertness control, added after `W7` | **Fired on its first outing.** `ARM-RLM-AFTER`'s column is identical to ARM-NONE's on every family — it did not fail, it never fired | `W7` produced the same shape (+0.00 everywhere while firing on 66% of positions) and it had to be read by eye |
| B2-P2 | Whether the residual is edge-placed, as predicted | **FALSIFIED — 49% against an 80% bar.** The mechanism story was incomplete | `B2-P1`, `B2-P3` and `B2-P4` held; `B2-P4` held at 100% against a 90% bar |
| W7-PREFIXCOMP | Whether prefix-aware completion can reach the forms `CorrectionEngine` accepts but `PredictiveEngine` can never offer | **NOTHING ADOPTED.** `append` **+0.00** at every prefix length on every register while firing on **66.2%** of positions; `interleave` **−9.58pp** on typed prefix-3 and breaks **11.54%** of top-1-correct positions | Ceiling measured at 1.98% of targets **before** the build. Reaching it requires displacing a correct literal completion, priced here at ~**−5 points per +1** |
| W7-LIMIT | Why `append` is inert | **`limit = 3` is the binding constraint, not the lexicon.** The strip is already full on essentially every position, so any append-style fallback is inert by construction | Worth knowing before the next fallback is proposed |
| W7-CLAUSE2 | A clause that could not fail under one mode | `append` cannot break top-1, so its 0.00 is not evidence. Both modes were run precisely so the clause became testable — under `interleave` it failed by **115×** | A clause that cannot fail is not a clause |
| W7-CORRECTION | Both "0-byte fixes" `A2` published | **Already shipped.** `A2` tested `token in he_lexicon.txt.gz`, which is not the shipped acceptance path: `PrefixStripper` already accepts **1.93%**, the bare abbreviation table **0.46%**, mark-stripping **0.21%** | **True out-of-lexicon on typed Hebrew is 5.52%, not 8.12%.** `A2`'s Rule 1 applied to the data and not to the code path |
| A2-OOVSHAPE | What the 7.61% out-of-lexicon gap on typed Hebrew is actually made of | **Hebrew words not in the lexicon 46.9%**, punctuation runs 22.1%, other 17.9%, geresh/gershayim 9.5%, **Latin 2.4%, emoji 0.3%** | 214,844 typed tokens. The intuitive answer — brand names, English, emoji — is **2.7% of the gap**. Measure the gap before sourcing for it |
| A2-LEVERS | What would close it, in points of typed tokens | **prefix chain already shipped 2.04pp** (upper bound) · **`""`→`״` normalisation 0.38pp** · hewiki titles 0.39pp at **+123,346 forms** · English lexicon 0.20pp · emoji 0.02pp | The two largest levers need **no external data** and together beat the best external source **6×** |
| A2-RESIDUAL | What is still out of lexicon after every lever | Dominated by **misspellings** — היתבוללות, לגיטמציה, גיזענות, התבללות | Not missing vocabulary: the errors the keyboard exists to correct. A lexicon that absorbed them would teach itself to accept them. **OOV is not a number to drive to zero** |
| A2-SEARCH | Whether the source search converged | 30 task-proxy queries → 53 distinct datasets; **21 of 30 returned nothing new**. Hebrew has **no** dataset indexed under hate, offensive, toxic, sarcasm, slang, forum, comments, whatsapp, sms, spelling or grammar | The empty queries are the finding. Without them, *"no closer corpus exists"* goes back to being an assumption. **LDC, ELRA, CLARIN and Zenodo are NOT SEARCHED** |
| A2-CORRECTION | A defect I reported and then withdrew | I first read **0%** of gershayim OOV as being in the shipped abbreviation table and called it an asset defect. **Parse error of mine** — the table is `bare<TAB>gershayim<TAB>freq` and I compared against the wrong column. Correct figure **65.4%** | The asset is sound. The real gap is normalisation, which is a code fix rather than a sourcing problem |
...
| W1-TYPED | Prediction on the first evaluation slice a person typed | **prefix-1 completion top-3 10.33%, 95% CI [9.75%, 10.92%]**, against 23.72% on transcribed dialogue and 5.43% on encyclopedic prose, all re-measured in the same run | 10,700 Ynet comments (MIT), ~20,000 positions per row, bootstrap over **sentences**. **Corrects the repository's headline register claim by ×2.3** — see `docs/PREDICTION_MEASUREMENTS.md`. `M10-REGISTER` unchanged |
| W1-ALPHABET-COST | What leaving the deleted characters in actually costs | **Not prediction quality — coverage.** Punctuation and Latin in the *context* cost **+0.02 points** on Hebrew targets (`W1-P2` FALSIFIED). Counting every token a person types, OOV rises **4.04% → 7.61%** and top-3 falls to 9.84% | **7.61% of typed tokens are words this engine has nothing to say about.** Both existing corpora score that at 0% by construction |
| W1-FALSEALARM | The real-word detector's flag rate on **correct** text, by register | **0.19% transcribed → 0.33% typed (×1.7)** | Moves the false-alarm side of a layer whose precision ceiling is 39.7% against a 40% withdrawal rule. `P7` in the definition of done already reads NOT MET |
| W1-PC1 | The positive control found a defect in the published comparison | **5.35% is the wiki slice's odd (test) half**; the whole slice is 5.43%, the even half 5.38%. 23.72% is a **whole** slice | All three are inside PC-1's tolerance so the control passed and no conclusion moves. Recorded rather than left to be found |
| W1-DISJOINT | Overlap between the typed slice and the OpenSubtitles training source | **87,457,198 lines streamed; 22 colliding keys; 46 comments dropped**, 10,746 → 10,700 | Compared on the Hebrew-token reduction, because the typed text is tokenized by its authors and an exact compare would never collide. **The Wikipedia half is NOT RUN** — that source is absent here — and rests on provenance |
| A1-ALPHABET | Whether transcribed and typed Hebrew share an alphabet | **They do not.** Over the lines the repo's own filter keeps: geresh/gershayim **0.01% → 3.57% (×604)**, brackets **0.21% → 5.71% (×27)**, emoji **0.00% → 1.27% (×458)**, Latin-or-digit **3.66% → 7.41%** | OpenSubtitles 3,000,000 lines (**prefix sample, not random**, streamed, never written to disk) vs Amram et al. Ynet comments, 12,804, MIT. **Neither register is phone typing** — `M10-REGISTER` unchanged |
| A1-FILTER | Whether removing the `[א-ת]+` filter is the repair | **It is not.** On the corpus we have it recovers 3.66% of lines for script-mixing and 0.21% for brackets. The source is the problem, not the filter | Reorders the plan: `docs/PROGRAM.md` W1 replaces un-filtering with a typed corpus |
| A1-BLINDSPOT | Three shipped features evaluated against text that cannot contain their subject | Abbreviation handling built around geresh/gershayim is measured where the correct character appears in **1 line in 17,000** against 1 in 28 of typed Hebrew; the bracket category `B1` hand-built appears in 5.71% of typed comments; emoji, which `GraphemeSegmenter` and backspace width exist for, appear **never** | Same run. Not a new hole — the quantity behind the hole `H1` reported as 0.00% |
...
| E2-HEAPFLOOR | Where the runtime memory actually is | **Shipped warm-up floor is 27 MB over a 2 MB empty JVM. Lexicon +15 MB, trie +6 MB, bigrams +4 MB.** Trie arrays 7,948,738 B; lexicon arrays 6,029,781 B | Smallest `-Xmx` each cumulative load survives, one JVM per (stage, heap) pair, plus an `empty` zero arm. Deltas are marginal, **not retained sizes** — the trie's is smaller than its own arrays |
| E2-COPYPATH | The first E2 figures measured a path the app does not take | **CORRECTED.** The probe copied the lexicon into an `ArrayList<String>`; `CorrectionController` passes `lexicon.asWordList()`, a view. Published 47 MB total / +28 MB trie; actual **27 MB / +6 MB** | Same failure as `A2` and `W7`. Discarded arm kept as stage `trie-copy` (+22 MB) rather than deleted, and it reproduces the ~34 MB copy cost `asWordList()`'s KDoc already recorded |
| E2-WRONGTARGET | What `E1`'s compression plan would have freed | **Almost no memory.** It targets the bigram table, the largest thing the app **ships**. Corrected, the table is +4 MB of 27 — still not the largest thing held | APK bytes and runtime RAM are different budgets with different largest items, and only the second enables features |
| E2-NOTRAP | Why `E1`'s deflate trap does not apply to the trie | **The trie has no shipped form.** Built at runtime, so a succinct form competes against 7.95 MB of arrays, never against deflate | `E1`'s 19-bit packing was worthless because the APK already deflates. That reasoning does not transfer — unchanged by the correction |
| E2-BUDGET | What the freed memory would be worth | `K1` measured an int8 model at **548,992 B** for 16,384 units. **The trie's arrays are ~14× that budget**, not the 51× first published | Recovering three quarters of it frees ~11× what `K1`'s model needs. Worth having; no longer the dominant term |
| E2-NOTYET | What `E2` does **not** establish | **No succinct trie has been built here**, and the loader-transient reading of the lexicon's +15 MB is inferred from `readBytes()`, **not measured** | Build-host JVM with SerialGC, not Android's ART. **`M7-LAT` still never run** |
| E3-SIZE | A succinct trie built and measured on this lexicon | **802,925 B against `LexiconTrie`'s 7,948,738 B — 9.90x.** Bar was <1,500,000 B and >=4x | Balanced parens, 2 bits/node, plus a byte label array and a terminal bitvector. The three id arrays are deleted, not compressed |
| E3-SHAPE | Why balanced parens and not LOUDS | **LOUDS renumbers into BFS order**, breaking the terminal-rank property and forcing a 355,587-entry map (~1.42 MB) — more than the shape it replaced | Pre-order numbering is what makes `firstChild[v] == v+1` and `wordIndex == terminal rank`. The textbook choice was wrong here |
| E3-EQUIV | Whether the succinct form answers identically | **2,000 searches byte-identical; 355,587 / 355,587 words accepted** | Built independently from the same sorted list, not converted, so this compares two constructions |
| E3-CONTROL | That the equivalence and acceptance checks can fail | **PC-1 RED, PC-2 RED, PC-3 acceptance GREEN + equivalence RED** | PC-3 is the load-bearing one: an extra accepted word is invisible to acceptance, so the two checks are proven distinct, not one check twice |
| E3-FLOOR | Heap floor with the succinct trie substituted | **Whole chain 27 MB -> 19 MB. Trie stage 23 MB -> 17 MB: marginal cost ZERO.** Predicted 18-21 MB | **Prediction FALSIFIED in the favourable direction** and recorded as falsified. The structure fits in slack the lexicon load already needed |
| E3-LATENCY | What it costs on the input path | **2.51x slower: 1.415 -> 3.550 ms/search.** Build host, 300 perturbed queries x3 after warm-up | **Unregistered** — no bar was committed, so none is applied. Measured because the obvious objection must not go unmeasured |
| E3-NOTSHIPPED | Whether this replaces `LexiconTrie` | **No.** Size and equivalence cleared their bars; the input path got 2.51x slower and **`M7-LAT` has still never run**, so there is no latency bar to clear | `completions`/`completionsTopK` were not ported either. The 8 MB is established as real and available, not taken |
| E3-ALPHABET | The lexicon's character set | **Exactly 27: U+05D0-U+05EA, contiguous. No geresh, gershayim, hyphen or apostrophe** | Structural, not coverage: `A2`'s 65.4% OOV gershayim forms **cannot** be represented at all, so adding words would not close that gap |
| E1-ASSETCOST | What the APK actually pays for its assets | `he_bigrams.bin` **1,682,421 B in the APK**, not the 2,697,304 B it unpacks to. Assets are **2,862,795 of 5,121,195 B**; the bigram table alone is **32.8% of the APK** | **Corrects a claim I made**: the APK is a zip and deflates its assets, so every saving must be counted against the compressed figure |
...
| E4-EMULATOR | Whether NOTICE 6's "structural limit" claim is true | **Half false, and it had never been tested.** An emulator boots Android 36 here in ~15 min, the app installs, and `ime set` makes it the system IME — which `M7-LAT` recorded as needing `WRITE_SECURE_SETTINGS` a benchmark cannot get. What is missing is KVM: no `/dev/kvm`, no `vmx`/`svm`. Unaccelerated, the app ANRs at startup (load 39.11, CPU stall 90%). **THREE configurations tried**: 4 vCPUs cut boot 15min->5min and AOSP-without-GMS made guest load fall instead of rise — both real improvements, neither sufficient. On the lightest image, 4 cores, idle host, **the launcher itself ANR'd** | **Zero rows closed** — only `dictionary_reloads=1` accumulated; the keyboard never laid out. The ANR is the emulated CPU, **not an app defect**, and is not recorded as one. The 22 rows do not share one blocker: geometry is emulator-safe, timing and TEE and perception are not |
| E4-APPTESTS | Automated coverage of the `:app` module | **Was zero; now 5.** `app/src/test` did not exist and Robolectric was not a dependency, while `:core` carried 303 tests and `:app` is where everything is wired. Robolectric added at **test scope only** — it cannot reach the shipped artifact | Covers service construction, and that a password field is counted restricted with `initialTextTaken` **unchanged**, plus a control that a plain field is not. Asserted on the recorded BOOLEAN, never on the text: naming `getInitialTextBeforeCursor` trips `priv.initial_text`, whose allow-list says *widening this is a privacy decision, not a refactor* — so the test was rewritten, not the gate. **Closes no device-blocked row**: shadows are a reimplementation, and `M4-DEVICE` asks what the REAL framework hands over. Two more cover the user-facing screens: `OnboardingActivity` (the `exported="true"` entry point) and `SettingsActivity` each open without throwing AND attach a non-empty composition — the non-vacuity check, without which they pass against an `onCreate` that does nothing. A crash on first tap had nothing standing between it and a release except someone opening the screen by hand |
