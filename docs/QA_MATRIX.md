...
| E2-HEAPFLOOR | Where the runtime memory actually is | **`LexiconTrie` costs +28 MB of a 47 MB floor; the bigram table costs +2 MB.** Lexicon 17 MB, trie 567,767 nodes | Smallest `-Xmx` each cumulative load survives, one JVM per (stage, heap) pair. **Over-states retained size by collector headroom** — a lower bound on what a device must supply |
| E2-WRONGTARGET | What `E1`'s compression plan would have freed | **Almost no memory.** It targets the bigram table, which is the largest thing the app **ships** and the **smallest** thing it holds | APK bytes and runtime RAM are different budgets with different largest items, and only the second enables features |
| E2-NOTRAP | Why `E1`'s deflate trap does not apply to the trie | **The trie has no shipped form.** It is built at runtime from the lexicon text, so a succinct representation competes against 28 MB of pointers, never against deflate | `E1`'s 19-bit packing was worthless because the APK already deflates. That reasoning does not transfer |
| E2-BUDGET | What the freed memory would be worth | `K1` measured an int8 model at **548,992 B** for 16,384 units. **The trie alone is ~51× that entire budget** | Recovering a fifth of it would free more than forty times what `K1`'s model needs |
| E2-NOTYET | What `E2` does **not** establish | **No succinct trie has been built here.** The reductions quoted are what the literature reports for the technique, not a measurement on this lexicon | Build-host JVM with SerialGC, not Android's ART. **`M7-LAT` still never run** |
