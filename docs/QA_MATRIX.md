...
| GATE-API-1 | No IME API that compiles cleanly and fails at runtime (§1.1/1.3/1.4/1.6) | 110 files, 6 rules | Planted session-override, return-value branch, hardcoded backspace, blocking fetch |
| GATE-API-1 | `getInitial*` accessors only inside the privacy boundary (§1.2) | 110 files | Planted read outside the boundary |
| GATE-API-1 | Nothing typed reaches logcat or stdout | 110 files, production sources | Planted `Log.d` and `println` |
| GATE-CRYPTO-1 | No ECB, hardcoded IV/key, seeded `SecureRandom`, broken primitive | 110 files, 4 rules | Planted `AES/ECB`, fixed IV, hardcoded key, MD5, seeded random |
...
| GATE-LEARN-2 | Learning happens in exactly one place, guarded by `session.mayLearn` | 114 Kotlin source files | A second, unguarded call site |
